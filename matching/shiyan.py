...
#查询结果
query = es.search(index='essay',body=query_json,scroll='15m',size=10000)
...
def match_es_word(text):
    ...
    print(match_sort)
    max_number = heapq.nlargest(100, match_sort)    # 拿到前100个元素，并排序
    max_index = []
    for t in max_number:
        #找到索引位置
        index = match_sort.index(t)
    ...
    # match_index = map(match_sort.index, heapq.nlargest(10, match_sort))
    # num_index=list(match_index)
    # print(re1)
    # num_index1 = list(set(num_index))
    # num_index1.sort(key=num_index.index)

        # print(num_index1[i])
        # print(abstdata[num_index1[i]])
